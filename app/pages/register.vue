<template>
  ...
  >
    <div class="flex flex-col gap-6 md:gap-8">
      ...
      <form @submit.prevent="handleSubmit" class="space-y-4 md:space-y-6">
        <div class="flex flex-col gap-4 md:gap-6">
          ...
          <UIInputText
            ...
            :rules="[
              (v) => !!v || $t('field_required', { field: $t('phone_number') }),
              (v) =>
                /^\d{10}$/.test(v) ||
                $t('field_invalid', { field: $t('phone_number') }),
            ]"
            class="w-full"
          />
        ...
        <button
          type="submit"
          class="w-full bg-white h-12 md:h-14 lg:h-16 rounded-xl md:rounded-2xl text-black font-bold text-sm md:text-base mt-6 md:mt-8 hover:brightness-90 transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-white/20"
        >
          {{ $t("sign_up") }}
        </button>
      </form>
      ...
      >
        <p class="text-center">{{ $t("have_account") }}</p>
        <button
